...
every simulatable draw). **The build regenerates both**, training the classifier and running
...
`--draw` takes a **`tournament_id`**, which is the field inside the draw JSON and not always the filename stem. Two are shipped today, both simulatable: `ausopen_2026_atp_full` (in `ausopen_2026_atp_full.json`, the default precomputed draw) and `usopen_2024_atp_full` (in `example_usopen_2024_full.json`). Passing an unknown id fails with the registry's own list of what *is* addressable.
...
it runs unattended. Its output changes what the public demo shows, since a refresh
...
0. **Coverage.** `--end` (default: the calendar year) must not exceed
   ...
   **This is the one gate guaranteed to fire on its own, and the one most likely
   ...
   deliberately decoupled, since `2026` is a partial season). The abort message names
...
the next run overwrites it. The window is also small, because the write is the last
...
runner, and `metadata.estimator_class` in each cache file records which of the
...
4. Convert to CSV with `Date` as `YYYY-MM-DD` and commit it. Reading the vendor's
   ...
   add it to `requirements.txt`**, or CI and both Docker images would then install
...
per-row name→id resolution to reproduce, which is not covered here. The report prints the
