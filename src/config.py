...
RAW_DATA_DIR = Path("data/raw")         # Vendored per-year CSVs; read offline by the loader
...
# boundary in the CLI. Kept distinct from the two 0.5s below: they happen
...
# Seed for that swap's RNG (was previously hardcoded in preprocess.py).
PLAYER_SWAP_SEED = 42
...
# (common/names.py). Promoted here from a hardcoded 0.6 in the CLI.
FUZZY_MATCH_CUTOFF = 0.6

# ==========================================
# SERVE/RETURN SKILL TABLE
# ==========================================
...
P_MIN = 0.50
P_MAX = 0.90
# Skill-gap amplification factor for the point-win model. The §1 additive
...
# Slam straight-set share. A constant-p i.i.d. match structurally
...
# to bring the bo5/bo3 set-count distributions into the target tolerance bands while
# keeping games/set and tiebreak frequency in range.
POINT_GAP_GAMMA = 1.9
...
# draw validator (whose JSON field is spelled `final_set_tiebreak`) and
...
# simulations". ~1,000 is enough for a ±1.5pp standard error while keeping a
# manual sanity run to a couple of seconds.
...
# NOT RECONCILE_MODE.
#
# "blend" gives the point model half the say in the match-win probability, which
# is the configuration the scoreline-realism validation (scripts/validate_sim.py)
# was run under. Moving these callers to "classifier_anchor" would hand every
# winner to the classifier alone and would need its own validation pass, a
# deliberate open question, not an oversight. config.RECONCILE_MODE remains the
# system-wide default.
...
# ace-01-architecture.md ("tens of seconds" for a 128 draw), which the bracket
# simulator's prob_cache is what actually makes achievable.
MC_RUNS = 5000
# Default base seed for that run. Every entry point must accept a seed
# (the determinism rule); this is the value used when a caller does not
...
#                              would. Checking a production build against a
#                              local API is the one thing the deploy docs ask a
#                              developer to do, and without this entry every
#                              request fails at the preflight.
#
# A deployed frontend's origin is NOT here, whoever deploys the API must add it
# explicitly. Tests index this list positionally, so append rather than reorder.
API_ALLOWED_ORIGINS = [
    "http://localhost:5173",
    "http://127.0.0.1:5173",
    "http://localhost:4173",
...
#
# It points at common/classifier_adapter.py, a UI-free module the API can
# import outright, so no cli/ module is imported by the server at all
# (ace-04-current-state.md §7 seam 7, now closed). It stays a *string* because
# which model a deployment publishes is configuration, and every response reports
# the adapter that actually ran in `metadata.adapter`, derived from the resolved
# factory.
...
# different story asks for one by passing ?seed=.
...
# a deliberate, bounded exception to the "aggregate MC is cache-only" rule,
...
# ---- Shared concurrency cap across the live-compute endpoints ----
...
# oversubscribe that CPU budget several times over, so the
...
# named <tournament_id>.json. A full 128 x 5,000 job takes ~29 s, which the
...
# ==========================================
# MARKET-ODDS BENCHMARK, EVALUATION ONLY
# ==========================================
...
# vendor's .xlsx to .csv once, by hand. Refreshing it is a manual step; no
# automated script fetches it.
