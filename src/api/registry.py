"""Tournament draw registry, what ``data/draws/`` actually contains.
...
**It reuses the draw loader wholesale.** Every file goes through
...
**Addressing.** A valid draw is keyed by its own ``tournament_id``. An *invalid*
one has no trustworthy id, the field may be missing,
...
transport concern, and keeping it framework-free means an offline script (the
precompute) can resolve an id → draw without importing the web app:
``scripts/precompute_sim.py`` builds a registry to turn its ``--draw
...
class TournamentEntry:
    ...
    def is_simulatable(self) -> bool:
        """True when this draw can actually be simulated.
        ...
        **Loadable is not the same as simulatable**, and both consumers of this
        ...
        The rule is the simulator's: ``simulate_bracket`` refuses a bracket
        containing placeholder entrants, because a placeholder has no
        ``player_id`` and no classifier history, so no match-win probability
        exists for it. This property reads the ``is_placeholder`` flag to reach the same verdict
...
def cache_path_for(
    ...
    """Where ``tournament_id``'s precomputed simulation lives.
...
def build_registry(
    ...
    Args:
        skill_table: id-keyed skill table, used by ``load_draw`` to resolve entrant
