"""Startup-loaded application state for the API.

Everything expensive the API needs, the vendored data, the leakage-safe
histories, the skill table and the pinned classifier, is loaded **once**, at
app startup, into an immutable :class:`ApiContext` stored on ``app.state``. No
request handler ever loads a file, and nothing here reaches the network. This is
the Phase 3 global constraint: *load the skill table + classifier once at
startup, not per request.*

**Why this is not simply ``cli.simulate_match.build_context``.** That function
builds the same startup context for the CLIs, but it lives in ``cli/``, so
importing it would put ``api/ → cli/`` into the dependency graph, which the
layering rule forbids. This module reuses every *core* step verbatim
...
class ApiContext:
    ...
    Attributes:
        ...
        skill_table: The id-keyed serve/return skill table, the API's
        ...
        estimator: The pinned classifier, an opaque ``predict_proba`` estimator
            ...
            ``ace-04-current-state.md §4``). **Not called by any endpoint**,
...
def build_api_context(
    ...
    Reads only vendored files, no network. Measured at
