...
# slots is refused by monte_carlo by design (T2.2), so precomputing one is not a
...
# real build; ace-04-current-state.md §7 seam 9). So a compose `environment:`
...
# is NOT a node image running `vite preview` (a dev-facing command — T4.5's
# explicit finding, seam 9). nginx's stock config serves /usr/share/nginx/html
