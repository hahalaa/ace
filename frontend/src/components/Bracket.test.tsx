...
describe('rendering a toy 8 draw', () => {
  ...
  it('fetches through the API client, once, for the requested id', async () => {
    await renderBracket(toyEightDraw());
...
describe('non-storybook rendering is unchanged', () => {
  ...
  it.each(cases)('renders %s exactly as before', async (_name, response) => {
    const { container } = await renderBracket(response);
    expect(container.innerHTML).toMatchSnapshot();
  });

  it('renders a single filtered round exactly as before', async () => {
    const { container } = await renderBracket(toyEightDraw());
    fireEvent.click(
  ...
  it('renders the loading status exactly as before', () => {
  ...
  it.each(failures)('renders the %s panel exactly as before', async (_name, error) => {
  ...
  it('renders the unlayoutable-draw refusal exactly as before', async () => {
    getBracketMock.mockResolvedValue(toyEightDraw({ draw_size: 12 }));
    const { container } = render(<Bracket tournamentId="toy_8" />);
    await screen.findByRole('alert');
    expect(container.innerHTML).toMatchSnapshot();
