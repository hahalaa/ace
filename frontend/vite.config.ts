...
//
// T4.5 reviewed this file for the static-build/deploy path and changed **no
// build option**: every default is already the one a static host wants.
// Recorded here so the next reader does not re-derive it, and so a future
// change is a deliberate departure rather than an accident.
//
...
export default defineConfig(({ command, mode }) => {
  if (command === 'build') {
    ...
    if ((env.VITE_API_BASE_URL ?? '').trim() === '') {
      ...
      // CI build-check command (T5.2), and that job has no API to point at; a
